...
function CandidateCard({
  ...
  return (
    <Card className="group p-5 transition-all duration-200 hover:border-black/20 hover:-translate-y-0.5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="font-medium text-zinc-800">{candidate.member.name}</div>
      ...
      <div className="mt-4 flex flex-wrap gap-1.5 text-[11px] text-zinc-500">
        <span className="rounded-md bg-black/5 px-1.5 py-0.5">{missions.length} missions</span>
        ...
        <span className="rounded-md bg-black/5 px-1.5 py-0.5">{modules.size} modules touched</span>
      </div>
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl px-6">
        <section className="py-20 text-center sm:py-28">
          ...
          <h1
            className="mx-auto mt-6 max-w-3xl text-4xl font-semibold tracking-tight text-zinc-950 sm:text-6xl animate-fade-up"
            style={{ animationDelay: "60ms" }}
          >
            The interviewer that{" "}
            <span className="bg-gradient-to-r from-aurora-600 via-aurora-500 to-mint-600 bg-clip-text text-transparent">
              knows what you built
            </span>
          </h1>
          <p
            className="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-zinc-500 sm:text-lg animate-fade-up"
        ...
        <section id="start" className="scroll-mt-10 py-12">
          <h2 className="text-lg font-medium text-zinc-800">Pick a candidate</h2>
          ...
          {showAll ? (
            ...
            <button
              ...
              className="mt-6 text-sm font-medium text-aurora-600 hover:text-aurora-600 transition-colors"
            >
        ...
        <section id="how" className="scroll-mt-10 py-16">
          <h2 className="text-lg font-medium text-zinc-800">Why the assessment is worth trusting</h2>
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {STEPS.map((step) => (
              <Card key={step.icon} className="p-6">
                ...
                <div className="mt-3 font-medium text-zinc-800">{step.title}</div>
        ...
        <footer className="border-t border-black/10 py-10 text-center text-xs text-zinc-400">
          VIVA · ABTalks Hackathon · backend at <span className="text-zinc-400">/api/interview</span>
        </footer>
      </main>
    </div>
  );
